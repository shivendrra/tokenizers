...
# tokenizer.train_parallel(target_vocab=5000, num_processes=4)
...
def main():
    ...
    tokenizer.train_parallel(target_vocab=5000, num_processes=6)
    tokenizer.save_model(name)
    time03 = timeit.default_timer()

    print(f'total time taken: {((time03 - time01)/60):.2f} mins')
    print(text == tokenizer.decode(tokenizer.encode(text)))
